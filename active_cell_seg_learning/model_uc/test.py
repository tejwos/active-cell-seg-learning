...
def test(loader, model, device = "cpu", classes = 2):
    """
    ...
    print(
        f"Accuracy: Got {numCorrect}/{numPixels} with acc {accuracyValue/ countValue: .2f} "
        )
    print(
        f"Lose Score:{loseScore / len(loader): .2f} "
        )
    print(
        f"IoU Score:{iouValue / len(loader): .2f} "
        )

    data = [(loseScore / len(loader)).item(), accuracyValue/ countValue, (numCorrect).item(), iouValue / len(loader)]
    return data